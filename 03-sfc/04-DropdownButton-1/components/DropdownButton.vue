<template>
  <div class="dropdown" :class="{'show': opened}" @click="opened = !opened">
    <button type="button" class="button dropdown__toggle" :class="{'dropdown__toggle_icon': hasIcon}">
      <app-icon v-if="current.icon" :icon="current.icon" />
      {{ title }}{{ current.text ? ' - ' + current.text : '' }}
    </button>

    <div class="dropdown__menu" :class="{'show': opened}">
      <button v-for="option in options" :key="JSON.stringify(option)" class="dropdown__item" :class="{'dropdown__item_icon': hasIcon}" @click="select(option.value)" type="button">
